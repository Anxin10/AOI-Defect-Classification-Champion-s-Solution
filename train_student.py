...
def train_one_epoch(model, optimizer, scheduler, dataloader, device, epoch, scaler, accum_iter, ema_model=None):
    ...
    pbar = tqdm(enumerate(dataloader), total=len(dataloader), desc=f"Student Train Epoch {epoch}")
...
def valid_one_epoch(model, dataloader, device, epoch):
    ...
    pbar = tqdm(enumerate(dataloader), total=len(dataloader), desc=f"Student Valid Epoch {epoch}")
    for step, (images, labels) in pbar:
        images = images.to(device)
        labels = labels.to(device)
...
def run_student_training(model_name, debug=False):
    seed_everything(Config.SEED)
    
    TARGET_BATCH_SIZE = Config.BATCH_SIZE
    ACTUAL_BATCH_SIZE = 16 # Optimized for Speed
    ...
    # Load the EXTENDED dataset (Pseudo Labels included)
    csv_path = os.path.join(Config.DATA_DIR, 'train_pseudo.csv')
    if not os.path.exists(csv_path):
        print(f"Error: {csv_path} not found. Please run inference_pseudo.py first.")
        return
    ...
    # Load original train.csv to know which IDs are original
    orig_train = pd.read_csv(Config.TRAIN_CSV)
    orig_ids = set(orig_train['ID'])
    
    # Assign folds ONLY to original data
    df['fold'] = -1
    df.loc[df['ID'].isin(orig_ids), 'is_original'] = True
    df.loc[~df['ID'].isin(orig_ids), 'is_original'] = False
    
    # Assign folds to original data
    orig_df = df[df['is_original'] == True].reset_index(drop=True)
    orig_df = get_folds(orig_df, n_folds=Config.N_FOLDS, seed=Config.SEED)
    
    # Map folds back to main df
    id_to_fold = dict(zip(orig_df['ID'], orig_df['fold']))
    df['fold'] = df['ID'].map(id_to_fold).fillna(-1)
    
    img_size = Config.IMG_SIZES[model_name]
    timm_name = Config.MODELS[model_name]
    
    print(f"Training Student {model_name} ({timm_name})")
    
    for fold in range(Config.N_FOLDS):
        print(f"--- Student Fold {fold} ---")
        
        # [Auto-Resume] 1. Check if Fold is already completed
        best_model_path = os.path.join(Config.MODEL_SAVE_DIR, f"student_{model_name}_fold{fold}_best.pth")
        log_path = os.path.join(Config.OUTPUT_DIR, f"student_{model_name}_fold{fold}_log.csv")
        ...
        train_df = df[(df['fold'] != fold) | (df['is_original'] == False)].reset_index(drop=True)
        valid_df = df[df['fold'] == fold].reset_index(drop=True)
        
        print(f"Train Size: {len(train_df)} (Original + Pseudo)")
        print(f"Valid Size: {len(valid_df)} (Original Only)")
        ...
        train_dataset = AOIDataset(
            train_df, 
            transforms=get_transforms(img_size, mode='student'), # STUDENT AUGMENTATIONS
            mode='train'
        )
        ...
        train_loader = DataLoader(
            train_dataset, 
            batch_size=ACTUAL_BATCH_SIZE, # Optimization
            sampler=train_sampler, # Use Sampler
            shuffle=False,         # Must be False when using Sampler
            num_workers=Config.NUM_WORKERS,
            pin_memory=True,
            drop_last=True, # Optimization
            persistent_workers=True # Speed Optimization
        )
        valid_loader = DataLoader(
            valid_dataset, 
            batch_size=ACTUAL_BATCH_SIZE * 2, # Optimization
            shuffle=False, 
            num_workers=Config.NUM_WORKERS,
            pin_memory=True,
            persistent_workers=True # Speed Optimization
        ...
        # Speed Optimization: Disable Gradient Checkpointing
        ...
        # Initialize EMA
        # Reduced decay from 0.9999 to 0.995 for small dataset
        ema_model = ModelEmaV2(model, decay=0.995, device=None)
        
        scaler = torch.amp.GradScaler('cuda') # Optimization
        ...
        last_model_path = os.path.join(Config.MODEL_SAVE_DIR, f"student_{model_name}_fold{fold}_last.pth")
        ...
        for epoch in range(start_epoch, Config.EPOCHS):
            ...
            # Validate using Original model (Optional)
            ...
            pd.DataFrame(history).to_csv(os.path.join(Config.OUTPUT_DIR, f"student_{model_name}_fold{fold}_log.csv"), index=False)
            
            if ema_valid_acc > best_acc:
                print(f"Student Accuracy Improved ({best_acc:.4f} -> {ema_valid_acc:.4f}). Saving EMA model...")
                best_acc = ema_valid_acc
                torch.save(
                    ema_model.module.state_dict(), 
                    os.path.join(Config.MODEL_SAVE_DIR, f"student_{model_name}_fold{fold}_best.pth")
                )
            ...
            torch.save({
                ...
                'best_acc': best_acc,
            }, os.path.join(Config.MODEL_SAVE_DIR, f"student_{model_name}_fold{fold}_last.pth"))
...
if __name__ == '__main__':
    ...
    run_student_training(args.model, args.debug)
